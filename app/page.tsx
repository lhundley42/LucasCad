...
const featureSummary = (feature: FeatureRecord) => feature.type === "fillet" ? `R${feature.radius ?? 0}` : feature.type === "chamfer" ? feature.method === "angle-distance" ? `${feature.distance ?? 0} mm · ${feature.angle ?? 45}°` : feature.method === "distance-distance" ? `${feature.distance ?? 0} × ${feature.distance2 ?? feature.distance ?? 0} mm` : `${feature.distance ?? 0} mm` : feature.type === "draft" ? `${feature.angle ?? 0}°` : feature.type === "shell" ? `${feature.thickness ?? 0} mm ${feature.outward ? "out" : "in"}` : feature.combine ?? "solid";
