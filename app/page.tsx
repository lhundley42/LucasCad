...
import type { SketchReference } from "./components/sketchConstraints";
...
const featureGlyph = (feature: FeatureRecord) => feature.type === "extrude" ? "▰" : feature.type === "revolve" ? "◉" : feature.type === "fillet" ? "◜" : feature.type === "chamfer" ? "◩" : "⌁";
const featureSummary = (feature: FeatureRecord) => feature.type === "fillet" ? `R${feature.radius ?? 0}` : feature.type === "chamfer" ? feature.method === "angle-distance" ? `${feature.distance ?? 0} mm · ${feature.angle ?? 45}°` : feature.method === "distance-distance" ? `${feature.distance ?? 0} × ${feature.distance2 ?? feature.distance ?? 0} mm` : `${feature.distance ?? 0} mm` : feature.type === "draft" ? `${feature.angle ?? 0}°` : feature.combine ?? "solid";
...
const flipSketchConstraint = (constraint: SketchConstraint): SketchConstraint => constraint.type === "mirror" ? constraint : constraint.type === "diameter" ? { ...constraint, position: flipPoint(constraint.position) } : { ...constraint, first: flipSketchReference(constraint.first), second: flipSketchReference(constraint.second), position: flipPoint(constraint.position) };
...
export default function Home() {
  ...
  const [contextMenu, setContextMenu] = useState<{ x: number; y: number; type: "sketch" | "feature" | "reference"; id: string } | null>(null);
  ...
  const [gridSquareInput, setGridSquareInput] = useState(String(DEFAULT_GLOBAL_SETTINGS.sketchGridSizeMm));
  ...
  const bodyFeaturePreview = useMemo<FeaturePreview | null>(() => {
    ...
    if (bodyFeatureDraft.type === "draft" && !bodyFeatureDraft.faceIndices.length) return null;
  ...
  }, [features, referenceGeometry, sketches]);
  ...
  useEffect(() => {
    ...
    const cancelPicking = (event: KeyboardEvent) => { if (event.key === "Escape") { setSketchSupportPicking(false); setProfileDialog(null); setRevolveAxisPicking(false); setBodyFeatureTool(null); setBodyFeatureDraft(null); setSolidSelectionMode(null); setSelectedDraftFaceIds([]); } };
  ...
  useEffect(() => {
    setGridSquareInput(String(Number(fromMillimeters(globalSettings.sketchGridSizeMm, globalSettings.unitSystem).toFixed(4))));
  }, [globalSettings.sketchGridSizeMm, globalSettings.unitSystem]);
  useEffect(() => {
    let visible = true;
  ...
  const editSketch = (id: string) => { const sketch = sketches.find((item) => item.id === id); setSketchViewDocument(cadDocument); setSketchView(defaultSketchView()); setSketchViewRotated(false); setEditingSketchId(id); setSelectedSketchId(id); setSelectedFeatureId(null); setSelectedPlane(null); setSketchSupportPicking(false); setSelectedBodyId(sketch && typeof sketch.plane !== "string" && sketch.plane.kind === "face" ? sketch.plane.bodyId : null); setSelectedFace(null); };
  ...
  const commitGridSquareInput = () => {
    const displayedValue = Number(gridSquareInput);
    if (!Number.isFinite(displayedValue) || displayedValue <= 0) {
      setGridSquareInput(String(Number(fromMillimeters(globalSettings.sketchGridSizeMm, globalSettings.unitSystem).toFixed(4)))); return;
    }
    updateGlobalSettings({ ...globalSettings, sketchGridSizeMm: Math.max(0.1, toMillimeters(displayedValue, globalSettings.unitSystem)) });
  };
  ...
  const editFeature = (id: string) => {
    ...
    if (!feature) return;
    if (feature.type === "fillet" || feature.type === "chamfer" || feature.type === "draft") {
  ...
  const createBodyFeature = () => {
    ...
    const label = bodyFeatureDraft.type === "fillet" ? "Fillet" : bodyFeatureDraft.type === "chamfer" ? "Chamfer" : "Draft";
    const feature: FeatureRecord = {
      ...
      ...(bodyFeatureDraft.type === "fillet" ? { edgeIndices: bodyFeatureDraft.edgeIndices, radius: bodyFeatureDraft.radius }
        ...
        : { neutralFaceIndex: bodyFeatureDraft.neutralFaceIndex, faceIndices: bodyFeatureDraft.faceIndices, angle: bodyFeatureDraft.angle, reverse: bodyFeatureDraft.reverse }),
    };
  ...
  const openContext = (event: React.MouseEvent, type: "sketch" | "feature" | "reference", id: string) => { event.preventDefault(); event.stopPropagation(); setContextMenu({ x: event.clientX, y: event.clientY, type, id }); };
  ...
  return <main className="cad-shell" onPointerDown={() => { if (contextMenu) setContextMenu(null); if (settingsOpen) setSettingsOpen(false); }}>
        ...
        <div className="settings-control" onPointerDown={(event) => event.stopPropagation()}>
          ...
          {settingsOpen && <div className="settings-menu" role="dialog" aria-label="Global settings">
            ...
            <section className="settings-section compact-setting">
              <label htmlFor="grid-square-size"><span>Default grid square</span><div className="setting-number"><input id="grid-square-size" aria-label="Default grid square size" type="number" inputMode="decimal" min={globalSettings.unitSystem === "imperial" ? 0.01 : 0.1} step={globalSettings.unitSystem === "imperial" ? 0.01 : 0.1} value={gridSquareInput} onChange={(event) => setGridSquareInput(event.target.value)} onBlur={commitGridSquareInput} onKeyDown={(event) => { if (event.key === "Enter") event.currentTarget.blur(); if (event.key === "Escape") { setGridSquareInput(String(Number(fromMillimeters(globalSettings.sketchGridSizeMm, globalSettings.unitSystem).toFixed(4)))); event.currentTarget.blur(); } }}/><span>{unitSuffix(globalSettings.unitSystem)}</span></div></label>
    ...
    <section className={`workspace ${propertiesCollapsed ? "properties-collapsed" : ""}`}>
      ...
      <section className={`viewport ${editingSketch ? "sketch-mode" : ""}`} aria-label={editingSketch ? "Parametric 2D sketcher" : "Interactive 3D viewport"}>
        ...
        {!editingSketch && bodyFeatureTool && <div className="support-pick-callout solid-feature-callout"><strong>{bodyFeatureTool === "draft" ? solidSelectionMode === "draft-neutral" ? "Select neutral face" : "Select faces to draft" : `Select edges to ${bodyFeatureTool}`}</strong><span>{bodyFeatureTool === "draft" ? solidSelectionMode === "draft-neutral" ? "Click a planar face that stays fixed" : "Click one or more side faces · selected faces turn cyan · parallel faces are excluded" : "Click edges; Shift-click or keep clicking to add more"} · Esc to cancel</span></div>}
        ...
        {!editingSketch && <div className="viewport-help">{bodyFeatureTool ? bodyFeatureTool === "draft" ? solidSelectionMode === "draft-neutral" ? "Neutral plane: choose the fixed planar face" : "Faces to draft: click highlighted side faces; parallel faces are not selectable" : "Edges highlight under the pointer; click to toggle selection" : profileDialog ? eligibleProfileSketchIds.length ? "Highlighted sketches are available for profile selection" : "No sketch geometry is available; create and exit a sketch first" : sketchSupportPicking ? "Planar faces and origin planes are ready for sketch placement" : "Preselect geometry or choose a feature command"}</div>}
      ...
      <aside className="properties-panel">
        ...
        {!editingSketch && selectedFeature && <><div className="panel-heading"><strong>{selectedFeature.name}</strong><span className="feature-state">{selectedFeature.type}</span></div><div className="sketch-inspector">{selectedFeature.type === "extrude" || selectedFeature.type === "revolve" ? <><h3>Profile</h3><p>{sketches.find((sketch) => sketch.id === selectedFeature.sketchId)?.name}</p><h3>Result</h3><p>{selectedFeature.combine === "new" ? "New solid body" : `${selectedFeature.combine} with Body ${bodyIds.indexOf(selectedFeature.targetBodyId ?? "") + 1}`}</p></> : <><h3>Target</h3><p>Body {bodyIds.indexOf(selectedFeature.targetBodyId ?? "") + 1}</p><h3>Parameters</h3><p>{selectedFeature.type === "fillet" ? `${selectedFeature.edgeIndices?.length ?? 0} edges · R ${selectedFeature.radius ?? 0} mm` : selectedFeature.type === "chamfer" ? `${selectedFeature.edgeIndices?.length ?? 0} edges · ${featureSummary(selectedFeature)}` : `${selectedFeature.faceIndices?.length ?? 0} faces · ${selectedFeature.angle ?? 0}°`}</p></>}<button className="edit-profile" onClick={() => editFeature(selectedFeature.id)}>Edit feature</button></div></>}
        {!editingSketch && !selectedReference && !selectedSketch && !selectedFeature && selectedBodyId && bodyFeatureTool && <><div className="panel-heading"><strong>{bodyFeatureTool === "fillet" ? "Fillet" : bodyFeatureTool === "chamfer" ? "Chamfer" : "Draft"} selection</strong><span className="feature-state">Body {selectedBodyIndex + 1}</span></div><div className="body-properties"><h3>Graphics selection</h3><div className="body-stat"><span>{bodyFeatureTool === "draft" ? "Faces" : "Edges"}</span><strong>{bodyFeatureTool === "draft" ? bodyFeatureDraft?.type === "draft" ? bodyFeatureDraft.faceIndices.length : 0 : selectedEdges.length}</strong></div><p>{bodyFeatureTool === "draft" ? solidSelectionMode === "draft-neutral" ? "Choose the planar face that remains fixed." : "Choose each face that should taper away from the neutral face." : "Choose one or more solid edges. Selected edges remain highlighted while the preview is active."}</p></div></>}
        {!editingSketch && !selectedReference && !selectedSketch && !selectedFeature && selectedBodyId && !bodyFeatureTool && <><div className="panel-heading"><strong>Body {selectedBodyIndex + 1}</strong><span className="feature-state">Solid body</span></div><div className="body-properties"><h3>Body contents</h3><div className="body-stat"><span>Features</span><strong>{selectedBodyFeatures.length}</strong></div><div className="body-stat"><span>Status</span><strong>Selectable solid</strong></div><p>{selectedFace ? selectedFace.planar ? `Face ${selectedFace.faceIndex} is selected. Create a sketch to attach it to that planar surface.` : `${selectedFace.geometryType?.toLowerCase() ?? "Curved"} face ${selectedFace.faceIndex} is selected. Use Axis to capture its center axis when available.` : "Select a planar face in the 3D viewport to create a supported sketch."}</p>{selectedFace?.planar && <button className="primary-wide" onClick={() => startSketchOnPlane({ kind: "face", bodyId: selectedFace.bodyId, faceIndex: selectedFace.faceIndex, faceId: selectedFace.id })}>New sketch on selected face</button>}</div></>}
    ...
    {referenceDraft && <div className="cad-dialog feature-dialog feature-flyout reference-geometry-flyout" onPointerDown={(event) => event.stopPropagation()}><header><strong>{editingReferenceId ? `Edit ${referenceGeometry.find((reference) => reference.id === editingReferenceId)?.name}` : `New reference ${referenceDraft.type}`}</strong><button onClick={() => { setReferenceDraft(null); setEditingReferenceId(null); setLivePlaneOffset(null); }}>×</button></header><div className="reference-definition"><span>{referenceDraft.type === "plane" ? "▱" : referenceDraft.type === "axis" ? "╎" : "•"}</span><div><strong>{referenceDraft.sourceLabel}</strong><small>{referenceDraft.type === "plane" ? "Parallel / coincident plane reference" : referenceDraft.type === "axis" ? "Linear axis reference" : "Model-space construction point"}</small></div></div>
      {referenceDraft.type === "plane" && <><div className="reference-field"><span>Quick reference</span><div className="reference-quick-options" role="group" aria-label="Plane quick reference">{(["XY", "XZ", "YZ"] as const).map((plane) => { const active = referenceDraft.sourceLabel === `${plane} origin plane`; return <button key={plane} type="button" className={active ? "active" : ""} aria-pressed={active} onClick={() => { const base = originPlaneFrame(plane); setLivePlaneOffset(null); setReferenceDraft({ ...referenceDraft, baseOrigin: base.origin, normal: base.normal, xDir: base.xDir, sourceLabel: `${plane} origin plane`, offset: 0, flip: false }); }}>{plane}</button>; })}</div></div><label>Signed offset distance<div className="input-with-unit"><input aria-label="Reference plane offset" type="number" step={globalSettings.unitSystem === "imperial" ? 0.05 : 1} value={fromMillimeters(livePlaneOffset ?? referenceDraft.offset, globalSettings.unitSystem)} onChange={(event) => { setLivePlaneOffset(null); setReferenceDraft({ ...referenceDraft, offset: toMillimeters(Number(event.target.value), globalSettings.unitSystem) }); }}/><span>{lengthUnit}</span></div></label><div className="signed-offset-hint"><span>−</span> opposite normal <b>0</b> coincident <span>+</span> along normal</div><button type="button" className={`draft-direction ${referenceDraft.flip ? "reversed" : ""}`} onClick={() => setReferenceDraft({ ...referenceDraft, flip: !referenceDraft.flip })}><span>⇅</span><div><strong>Flip plane normal</strong><small>{referenceDraft.flip ? "Normal reversed" : "Original reference normal"}</small></div></button><div className="dialog-hint">Click any highlighted flat face or existing reference plane. The gold plane is a live preview; drag either cyan arrow to adjust its signed offset before applying.</div></>}
      {referenceDraft.type === "axis" && <><div className="reference-field"><span>Quick origin axis</span><div className="reference-quick-options" role="group" aria-label="Axis quick reference">{(["X", "Y", "Z"] as const).map((axis) => { const active = referenceDraft.sourceLabel === `Origin ${axis} axis`; return <button key={axis} type="button" className={active ? "active" : ""} aria-pressed={active} onClick={() => setReferenceDraft({ ...referenceDraft, origin: [0, 0, 0], direction: axis === "X" ? [1, 0, 0] : axis === "Y" ? [0, 1, 0] : [0, 0, 1], sourceLabel: `Origin ${axis} axis` })}>{axis}</button>; })}</div></div><div className="reference-coordinate-grid"><label>Direction X<input type="number" step="0.1" value={referenceDraft.direction[0]} onChange={(event) => setReferenceDraft({ ...referenceDraft, direction: [Number(event.target.value), referenceDraft.direction[1], referenceDraft.direction[2]] })}/></label><label>Y<input type="number" step="0.1" value={referenceDraft.direction[1]} onChange={(event) => setReferenceDraft({ ...referenceDraft, direction: [referenceDraft.direction[0], Number(event.target.value), referenceDraft.direction[2]] })}/></label><label>Z<input type="number" step="0.1" value={referenceDraft.direction[2]} onChange={(event) => setReferenceDraft({ ...referenceDraft, direction: [referenceDraft.direction[0], referenceDraft.direction[1], Number(event.target.value)] })}/></label></div><div className="dialog-hint">Click any highlighted edge for a coincident, center, or midpoint-tangent axis. Click a cylindrical, conical, or toroidal face to use its center axis. The amber line previews the result before creation.</div></>}
      {referenceDraft.type === "point" && <><div className="reference-coordinate-grid"><label>X<input type="number" value={displayedLength(referenceDraft.position[0])} onChange={(event) => setReferenceDraft({ ...referenceDraft, position: [toMillimeters(Number(event.target.value), globalSettings.unitSystem), referenceDraft.position[1], referenceDraft.position[2]] })}/></label><label>Y<input type="number" value={displayedLength(referenceDraft.position[1])} onChange={(event) => setReferenceDraft({ ...referenceDraft, position: [referenceDraft.position[0], toMillimeters(Number(event.target.value), globalSettings.unitSystem), referenceDraft.position[2]] })}/></label><label>Z<input type="number" value={displayedLength(referenceDraft.position[2])} onChange={(event) => setReferenceDraft({ ...referenceDraft, position: [referenceDraft.position[0], referenceDraft.position[1], toMillimeters(Number(event.target.value), globalSettings.unitSystem)] })}/></label></div><div className="dialog-hint">Preselect a face for its center or an edge for its midpoint, or enter an exact XYZ position.</div></>}
    ...
    {bodyFeatureDraft && <div className={`cad-dialog feature-dialog feature-flyout body-feature-flyout ${bodyFeatureDraft.type === "draft" ? "draft-feature-flyout" : ""}`} onPointerDown={(event) => event.stopPropagation()}>
      <header><strong>{editingFeatureId ? `Edit ${features.find((feature) => feature.id === editingFeatureId)?.name ?? "feature"}` : bodyFeatureDraft.type === "fillet" ? "Constant-radius fillet" : bodyFeatureDraft.type === "chamfer" ? "Edge chamfer" : "Neutral-plane draft"}</strong><button onClick={closeFeatureEditor}>×</button></header>
      <div className="solid-selection-summary"><span>{bodyFeatureDraft.type === "draft" ? "⌁" : bodyFeatureDraft.type === "fillet" ? "◜" : "◩"}</span><div><strong>Body {bodyIds.indexOf(bodyFeatureDraft.targetBodyId) + 1}</strong><small>{bodyFeatureDraft.type === "draft" ? `${bodyFeatureDraft.faceIndices.length} face${bodyFeatureDraft.faceIndices.length === 1 ? "" : "s"} to draft` : `${bodyFeatureDraft.edgeIndices.length} edge${bodyFeatureDraft.edgeIndices.length === 1 ? "" : "s"} selected`}</small></div></div>
      {bodyFeatureDraft.type === "fillet" && <label>Radius<div className="input-with-unit"><input aria-label="Fillet radius" type="number" min="0.001" value={displayedLength(bodyFeatureDraft.radius)} onChange={(event) => setBodyFeatureDraft({ ...bodyFeatureDraft, radius: enteredLength(event.target.value) })}/><span>{lengthUnit}</span></div></label>}
      {bodyFeatureDraft.type === "chamfer" && <>
        ...
        <label>{bodyFeatureDraft.method === "distance-distance" ? "Distance 1" : "Distance"}<div className="input-with-unit"><input aria-label={bodyFeatureDraft.method === "distance-distance" ? "Chamfer distance 1" : "Chamfer distance"} type="number" min="0.001" value={displayedLength(displayedBodyFeatureDraft?.type === "chamfer" ? displayedBodyFeatureDraft.distance : bodyFeatureDraft.distance)} onChange={(event) => { setLiveChamferDrag(null); setBodyFeatureDraft({ ...bodyFeatureDraft, distance: enteredLength(event.target.value) }); }}/><span>{lengthUnit}</span></div></label>
        {bodyFeatureDraft.method === "distance-distance" && <label>Distance 2<div className="input-with-unit"><input aria-label="Chamfer distance 2" type="number" min="0.001" value={displayedLength(displayedBodyFeatureDraft?.type === "chamfer" ? displayedBodyFeatureDraft.distance2 : bodyFeatureDraft.distance2)} onChange={(event) => { setLiveChamferDrag(null); setBodyFeatureDraft({ ...bodyFeatureDraft, distance2: enteredLength(event.target.value) }); }}/><span>{lengthUnit}</span></div></label>}
        {bodyFeatureDraft.method === "angle-distance" && <label>Angle<div className="input-with-unit"><input aria-label="Chamfer angle" type="number" min="1" max="89" step="1" value={displayedBodyFeatureDraft?.type === "chamfer" ? Number(displayedBodyFeatureDraft.angle.toFixed(2)) : bodyFeatureDraft.angle} onChange={(event) => { setLiveChamferDrag(null); setBodyFeatureDraft({ ...bodyFeatureDraft, angle: Math.max(1, Math.min(89, Number(event.target.value))) }); }}/><span>deg</span></div></label>}
      ...
      {bodyFeatureDraft.type === "draft" && <>
        ...
        <label>Draft angle<div className="input-with-unit"><input aria-label="Draft angle" type="number" min="0.1" max="88.9" step="0.5" value={bodyFeatureDraft.angle} onChange={(event) => setBodyFeatureDraft({ ...bodyFeatureDraft, angle: Math.max(0.1, Math.min(88.9, Number(event.target.value))) })}/><span>deg</span></div></label><button type="button" className={`draft-direction ${bodyFeatureDraft.reverse ? "reversed" : ""}`} onClick={() => setBodyFeatureDraft({ ...bodyFeatureDraft, reverse: !bodyFeatureDraft.reverse })}><span>⇅</span><div><strong>Reverse direction</strong><small>{bodyFeatureDraft.reverse ? "Reversed from neutral-face normal" : "Along neutral-face normal"}</small></div></button>
      </>}
      <div className="dialog-hint">{bodyFeatureDraft.type === "draft" ? "Step 2 is active: click one or more body faces in 3D. Selected draft faces turn cyan; the neutral face stays amber." : "Keep clicking highlighted edges to add or remove them."} The model preview updates immediately.</div>
      <div className="dialog-actions"><button onClick={closeFeatureEditor}>Cancel</button><button className="primary" disabled={bodyFeatureDraft.type === "draft" ? !bodyFeatureDraft.faceIndices.length : !bodyFeatureDraft.edgeIndices.length} onClick={createBodyFeature}>{editingFeatureId ? "Apply changes" : `Create ${bodyFeatureDraft.type}`}</button></div>
    </div>}
    {contextMenu && <div className="feature-context-menu" style={{ left: contextMenu.x, top: contextMenu.y }} onPointerDown={(event) => event.stopPropagation()}><div className="context-title">{contextMenu.type === "sketch" ? sketches.find((sketch) => sketch.id === contextMenu.id)?.name : contextMenu.type === "feature" ? features.find((feature) => feature.id === contextMenu.id)?.name : referenceGeometry.find((reference) => reference.id === contextMenu.id)?.name}</div>{contextMenu.type === "sketch" && <><button onClick={() => { editSketch(contextMenu.id); setContextMenu(null); }}>Edit sketch</button><button onClick={() => { setPlaneDialog({ mode: "edit", sketchId: contextMenu.id }); setContextMenu(null); }}>Change plane or face</button></>}{contextMenu.type === "feature" && <button onClick={() => editFeature(contextMenu.id)}>Edit feature</button>}{contextMenu.type === "reference" && <><button onClick={() => { editReferenceGeometry(contextMenu.id); setContextMenu(null); }}>Edit reference geometry</button><button className="delete-action" onClick={() => { deleteReferenceGeometry(contextMenu.id); setContextMenu(null); }}>Delete reference and dependents</button></>}{contextMenu.type !== "reference" && <button className="delete-action" onClick={() => deleteFromTree(contextMenu.type, contextMenu.id)}>Delete {contextMenu.type}{contextMenu.type === "feature" ? " and later features" : " and dependents"}</button>}</div>}
  </main>;
}
