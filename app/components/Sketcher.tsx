...
import { angularDimensionLayout, angularDimensionValue, constraintSupersedesOrthogonalProfileDimension, constraintSupersedesSegmentDimension, controlPointsForEntity, cycleLinearOrientation, defaultLinearDimensionPosition, defaultLinearOrientation, dimensionReferencePoints, linearDimensionLayout, linearDimensionValue, linearOrientationOptions, lineFor, nonOverlappingReferenceHitRadius, preferredAxisOrSketchLineTarget, referencePoint, targetPointForAngularValue, targetPointForLinearValue, validLinearDimensionPair, type ExternalSketchReference, type LinearDimensionConstraint, type LinearOrientation, type MirrorConstraint, type SketchConstraint, type SketchReference } from "./sketchConstraints";
...
type Tool = "select" | "line" | "centerline" | "rectangle" | "circle" | "ellipse" | "arc" | "spline" | "trim" | "extend" | "corner" | "mirror" | "linear-dimension" | "angular-dimension" | "perpendicular-constraint" | "horizontal-constraint" | "vertical-constraint";
...
export function Sketcher({ entities: controlledEntities, constraints: controlledConstraints = [], externalReferences = [], dimensionOffsets: controlledDimensionOffsets = {}, dimensionTextScale = 0.5, nodeDiameterPx = 4, highlightWidthPx = 1.2, gridSquareSize = 8, unitSystem = "metric", onChange, onConstraintsChange, onDimensionOffsetsChange, onFinish, view = DEFAULT_VIEW, viewRotated = false, onSnapNormal }: { entities?: SketchEntity[]; constraints?: SketchConstraint[]; externalReferences?: ExternalSketchReference[]; dimensionOffsets?: Record<string, Point>; dimensionTextScale?: number; nodeDiameterPx?: number; highlightWidthPx?: number; gridSquareSize?: number; unitSystem?: UnitSystem; onChange?: (entities: SketchEntity[]) => void; onConstraintsChange?: (constraints: SketchConstraint[]) => void; onDimensionOffsetsChange?: (offsets: Record<string, Point>) => void; onFinish: () => void; view?: SketchView; viewRotated?: boolean; onSnapNormal?: () => void }) {
  ...
  const finishChain = () => { setDraft([]); setDimensionReferences([]); setDimensionMessage(null); setExtendSource(null); setExtendMessage(null); setCornerSource(null); setCornerMessage(null); setPerpendicularSource(null); setPerpendicularMessage(null); setTool("select"); setSnap(null); };
  ...
  const onMove = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (draggingSelection) {
      ...
      const nextConstraints = draggingSelection.originalConstraints.map((constraint) => {
        ...
        const actual = positioned.type === "linear" ? linearDimensionValue(positioned.first, positioned.second, positioned.orientation, nextEntities) : angularDimensionValue(positioned.first, positioned.second, positioned.position, nextEntities);
        return { ...positioned, conflicted: Math.abs(actual - positioned.value) > (positioned.type === "linear" ? 0.01 : 0.1) };
      });
    ...
    const raw = pointFromEvent(event);
    if ((tool === "linear-dimension" || tool === "angular-dimension") && dimensionReferences.length === 2) { setCursor(raw); setSnap(null); return; }
  ...
  const constraintsForGeometry = (nextEntities: SketchEntity[], drivingId?: string, drivingValue?: number) => constraints.map((constraint) => {
    ...
    const actual = constraint.type === "linear" ? linearDimensionValue(constraint.first, constraint.second, constraint.orientation, nextEntities) : angularDimensionValue(constraint.first, constraint.second, constraint.position, nextEntities);
    return { ...constraint, conflicted: Math.abs(actual - constraint.value) > (constraint.type === "linear" ? 0.01 : 0.1) };
  });
  ...
  const constraintLabelPoint = (constraint: Exclude<SketchConstraint, MirrorConstraint>) => constraint.type === "linear" ? linearDimensionLayout(constraint.first, constraint.second, constraint.orientation, constraint.position, entities).label : angularDimensionLayout(constraint.first, constraint.second, constraint.position, entities).label;
  ...
  const resetTransientToolState = () => {
    setDraft([]); setExtendSource(null); setExtendMessage(null); setCornerSource(null); setCornerMessage(null); setPerpendicularSource(null); setPerpendicularMessage(null); setMirrorMessage(null); setDimensionReferences([]); setDimensionMessage(null);
  };
  ...
  return <div className={`sketcher-shell ${viewRotated ? "view-rotated" : ""}`} style={highlightStyle}>
    ...
    <svg ref={svgRef} className={`sketch-canvas tool-${tool} ${dragging ? "dragging-point" : ""} ${draggingDimension ? "dragging-dimension" : ""} ${selectionMarquee ? "marquee-selecting" : ""} ${draggingSelection ? "dragging-selection" : ""}`} viewBox={`${viewX} ${viewY} ${viewWidth} ${viewHeight}`} preserveAspectRatio="xMidYMid slice" onPointerMove={onMove} onPointerUp={finishDrag} onPointerCancel={finishDrag} onPointerDown={onCanvasPointerDown} onDoubleClick={() => tool === "spline" && finishSpline()} onContextMenu={(event) => event.preventDefault()}>
      ...
      {entities.map((entity) => { const referenceId = `line:${entity.id}`; const dimensionLineTool = tool === "linear-dimension" || tool === "angular-dimension" || tool === "perpendicular-constraint" || tool === "mirror"; const referenceChosen = dimensionReferences.some((reference) => reference.kind === "line" && reference.entityId === entity.id); return <g key={entity.id} className={`sketch-entity ${selectedEntityIds.includes(entity.id) ? "selected" : ""} ${extendSource?.entityId === entity.id || cornerSource?.entityId === entity.id || perpendicularSource === entity.id ? "extend-source" : ""} ${entity.construction ? "construction" : ""} ${dimensionLineTool && entity.type === "line" ? "constraint-selectable" : ""}`} onContextMenu={(event) => openConstraintShortcut(event, entity)} onPointerDown={(event) => handleEntityPointerDown(event, entity)}>
