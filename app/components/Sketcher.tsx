"use client";
import { IDENTITY_PROJECTION, projectionDeterminant, unprojectSketchPoint, projectSketchPoint, sketchBillboard, type PlaneProjection } from "./sketchProjection";
...
import { useDocumentHistory, useSnapshotHistory } from "./DocumentHistory";
import { undoRedoShortcut } from "./snapshotHistory";
import { RectangleToolMenu } from "./RectangleToolMenu";
import { rectangleCorners, type RectangleMode } from "./rectangleGeometry";
import { isTangentConstraint, makeTangentConstraint, synchronizeTangencies, tangentContact, tangentSatisfied } from "./sketchTangency";
import type { TangentConstraint } from "./sketchConstraints";
type SketchHistorySnapshot = { entities: SketchEntity[]; constraints: SketchConstraint[]; dimensionOffsets: Record<string, Point>; hiddenDimensionKeys: string[] };
...
type Tool = "select" | "sketch-check" | "line" | "centerline" | "rectangle" | "circle" | "ellipse" | "arc" | "spline" | "trim" | "extend" | "corner" | "radius" | "mirror" | "linear-pattern" | "rectangular-pattern" | "circular-pattern" | "linear-dimension" | "angular-dimension" | "diameter-dimension" | "radial-dimension" | "tangent-constraint" | "perpendicular-constraint" | "horizontal-constraint" | "vertical-constraint";
...
export type SketchView = { center: Point; zoom: number; projection?: PlaneProjection };
...
const isDimensionConstraint = (constraint: SketchConstraint): constraint is Exclude<SketchConstraint, MirrorConstraint | PatternConstraint | TangentConstraint> => constraint.type === "linear" || constraint.type === "angular" || constraint.type === "diameter" || constraint.type === "radial";
...
export function Sketcher({ entities: controlledEntities, constraints: controlledConstraints = [], externalReferences = [], dimensionOffsets: controlledDimensionOffsets = {}, hiddenDimensionKeys: controlledHiddenDimensionKeys = [], onHiddenDimensionKeysChange, dimensionTextScale = 0.5, nodeDiameterPx = 4, highlightWidthPx = 1.2, gridSquareSize = 8, unitSystem = "metric", initialTool = "select", onChange, onConstraintsChange, onDimensionOffsetsChange, onFinish, view = DEFAULT_VIEW, viewRotated = false, onSnapNormal }: { entities?: SketchEntity[]; constraints?: SketchConstraint[]; externalReferences?: ExternalSketchReference[]; dimensionOffsets?: Record<string, Point>; hiddenDimensionKeys?: string[]; onHiddenDimensionKeysChange?: (keys: string[]) => void; dimensionTextScale?: number; nodeDiameterPx?: number; highlightWidthPx?: number; gridSquareSize?: number; unitSystem?: UnitSystem; initialTool?: SketchInitialTool; onChange?: (entities: SketchEntity[]) => void; onConstraintsChange?: (constraints: SketchConstraint[]) => void; onDimensionOffsetsChange?: (offsets: Record<string, Point>) => void; onFinish: () => void; view?: SketchView; viewRotated?: boolean; onSnapNormal?: () => void }) {
  ...
  const [rectangleMode, setRectangleMode] = useState<RectangleMode>("corners");
  ...
  const [hiddenDimensionKeys, setHiddenDimensionKeys] = useState<string[]>(() => controlledHiddenDimensionKeys);
  ...
  const [tangentSource, setTangentSource] = useState<string | null>(null);
  ...
  const snapshot = useMemo<SketchHistorySnapshot>(() => ({ entities, constraints, dimensionOffsets, hiddenDimensionKeys }), [entities, constraints, dimensionOffsets, hiddenDimensionKeys]);
  const restoreSketchSnapshot = (next: SketchHistorySnapshot, publish = true) => {
    cancelDimensionEditOnBlur.current = true;
    setEntitiesState(next.entities); setConstraintsState(next.constraints); setDimensionOffsets(next.dimensionOffsets); setHiddenDimensionKeys(next.hiddenDimensionKeys);
    setEditingDimension(null); setEditingConstraintId(null); setDragging(null); setDraggingSelection(null); setDraggingConstraint(null); setDraggingDimension(null); setDraggingSplineHandle(null); setSelectionMarquee(null);
    dragStartRef.current = null; dragSnapRef.current = null; setSnap(null); clearSelection(); resetTransientToolState(); setTool("select");
    if (publish) { onChange?.(next.entities); onConstraintsChange?.(next.constraints); onDimensionOffsetsChange?.(next.dimensionOffsets); onHiddenDimensionKeysChange?.(next.hiddenDimensionKeys); }
  };
  const localHistory = useSnapshotHistory(snapshot, restoreSketchSnapshot, Boolean(dragging || draggingSelection || draggingConstraint || draggingDimension || draggingSplineHandle || trimGesture), !documentHistory);
  const activeHistory = documentHistory ?? localHistory;
  const undo = () => activeHistory.undo();
  const redo = () => activeHistory.redo();
  const restoredRevision = useRef(documentHistory?.revision);
  useLayoutEffect(() => {
    if (!documentHistory || restoredRevision.current === documentHistory.revision) return;
    restoredRevision.current = documentHistory.revision;
    restoreSketchSnapshot({ entities: controlledEntities ?? [], constraints: controlledConstraints, dimensionOffsets: controlledDimensionOffsets, hiddenDimensionKeys: controlledHiddenDimensionKeys }, false);
  }, [documentHistory?.revision]);
  ...
  const synchronizeLinkedGeometry = useCallback((next: SketchEntity[], previous = entities, activeConstraints = constraints) => {
    ...
    return synchronizeTangencies(synchronizePatternLinks(mirrored, activeConstraints.filter(isPatternConstraint), changed, externalReferences), activeConstraints);
  ...
  const pointFromEvent = (event: Pick<React.PointerEvent<SVGElement>, "clientX" | "clientY">): Point => {
    ...
    return unprojectSketchPoint(transformed, projection) ?? cursor;
  };
  const findSnap = (raw: Point, anchor?: Point, excludeEntityId?: string) => {
    ...
    const best = preferredSketchSnap(raw, candidates, snapTolerance, gridSnap, projectPoint);
  ...
  const deleteSelection = () => {
    ...
    if (selectedDimensionKeys.length) { const keys = [...new Set([...hiddenDimensionKeys, ...selectedDimensionKeys])]; setHiddenDimensionKeys(keys); onHiddenDimensionKeysChange?.(keys); }
  ...
  useEffect(() => {
    const keydown = (event: KeyboardEvent) => {
      ...
      if (!documentHistory && direction) { event.preventDefault(); if (direction === "undo") undo(); else redo(); }
    };
  ...
  const onMove = (event: React.PointerEvent<SVGSVGElement>) => {
    if (planeEdgeOn || (event.buttons & 6) !== 0) return;
    if (trimGesture) {
    ...
    if (draggingSelection) {
      ...
      const nextEntities = synchronizeTangencies(synchronizePatternLinks(mirroredEntities, draggingSelection.originalConstraints.filter(isPatternConstraint), entityIds, externalReferences), draggingSelection.originalConstraints);
      ...
      const nextConstraints = draggingSelection.originalConstraints.map((constraint) => {
        if (isTangentConstraint(constraint)) return { ...constraint, conflicted: !tangentSatisfied(constraint, nextEntities) };
  ...
  const runSketchCheck = () => {
    if (tool === "sketch-check" && sketchCheck?.openEndpoints.length) {
      ...
      const nextHiddenKeys = hiddenDimensionKeys.filter((key) => ![...removeIds].some((entityId) => key === entityId || key.startsWith(`${entityId}:`))); setHiddenDimensionKeys(nextHiddenKeys); onHiddenDimensionKeysChange?.(nextHiddenKeys);
  ...
  const finishDrag = (event?: React.PointerEvent<SVGSVGElement>) => {
    ...
    if (selectionMarquee) {
      const end = event ? pointFromEvent(event) : selectionMarquee.current; const box = normalizedSelectionBox(projectPoint(selectionMarquee.start), projectPoint(end));
      const selectedIds = entities.filter((entity) => entityInSelectionBox(entity, selectionMarquee.start, end, projectPoint)).map((entity) => entity.id);
      setSelectedEntityIds(selectedIds);
      if (isPatternTool(tool) && patternDraft) {
        setSelectedConstraintIds([]); setSelectedDimensionKeys([]);
        setPatternDraft({ ...patternDraft, seedIds: selectedIds, stage: selectedIds.length ? patternDraft.kind === "circular-pattern" ? "center" : "direction-1" : "entities" });
        setPatternMessage(selectedIds.length ? `${selectedIds.length} entities selected · ${patternDraft.kind === "circular-pattern" ? "select the center" : "select Direction 1: sketch line, background edge, or red/green axis"}` : "Drag a selection box around the geometry to pattern");
      } else if (tool === "mirror") {
        ...
        setSelectedConstraintIds(constraints.filter(isDimensionConstraint).filter((constraint) => pointInSelectionBox(projectPoint(constraintLabelPoint(constraint)), box)).map((constraint) => constraint.id));
        setSelectedDimensionKeys(visibleDimensions.filter((dimension) => pointInSelectionBox(projectPoint(dimensionPosition(dimension)), box)).map((dimension) => dimension.key));
      }
    ...
    if (draggingSplineHandle) {
      ...
      dragStartRef.current = null; setDraggingSplineHandle(null); return;
    }
    if (!dragging) return;
    ...
    dragStartRef.current = null; dragSnapRef.current = null; setDragging(null); setSnap(null);
  };
  ...
  const onCanvasPointerDown = (event: React.PointerEvent<SVGSVGElement>) => {
    ...
    setSplineContextMenu(null);
    if (tool === "select" || tool === "mirror" && mirrorStage === "entities" || isPatternTool(tool) && patternDraft?.stage === "entities") {
    ...
    if (tool === "rectangle") {
      if (!draft.length) { setDraft([point]); return; }
      const corners = rectangleCorners(draft[0], point, rectangleMode);
      const a = corners[0]; const b = corners[2];
  ...
  const applyConstraintDimension = () => {
    ...
    if (!constraint || !isDimensionConstraint(constraint) || !Number.isFinite(enteredValue)) { setEditingConstraintId(null); return; }
  ...
  const applyDimension = () => {
    ...
    next = synchronizeLinkedGeometry(next);
  ...
  const marqueeBox = selectionMarquee ? normalizedSelectionBox(projectPoint(selectionMarquee.start), projectPoint(selectionMarquee.current)) : null;
  const editingDimensionPosition = editingDimension ? dimensionPosition(editingDimension) : null;
  const editingConstraint = constraints.find((constraint): constraint is Exclude<SketchConstraint, MirrorConstraint | PatternConstraint | TangentConstraint> => constraint.id === editingConstraintId && isDimensionConstraint(constraint)) ?? null;
  ...
  const hasConstraintConflict = constraints.some((constraint) => isTangentConstraint(constraint) ? !tangentSatisfied(constraint, entities) : "conflicted" in constraint && constraint.conflicted);
  const viewWidth = VIEW.width / safeZoom; const viewHeight = VIEW.height / safeZoom;
  const rootX = view.center.x - viewWidth / 2; const rootY = view.center.y - viewHeight / 2;
  const planeCorners = [{ x: rootX, y: rootY }, { x: rootX + viewWidth, y: rootY }, { x: rootX, y: rootY + viewHeight }, { x: rootX + viewWidth, y: rootY + viewHeight }].map((p) => unprojectSketchPoint(p, projection) ?? p);
  const viewX = Math.max(-1e6, Math.min(...planeCorners.map(p => p.x)) - gridSquareSize);
  const viewY = Math.max(-1e6, Math.min(...planeCorners.map(p => p.y)) - gridSquareSize);
  const planeWidth = Math.min(2e6, Math.max(...planeCorners.map(p => p.x)) - viewX + gridSquareSize);
  const planeHeight = Math.min(2e6, Math.max(...planeCorners.map(p => p.y)) - viewY + gridSquareSize);
  const highlightStyle = { "--sketch-highlight-width": `${highlightWidthPx}px` } as CSSProperties;
  const patternReferenceChosen = (kind: "entity" | "external", id: string) => [patternDraft?.direction1, patternDraft?.direction2].some(reference => reference?.kind === kind && (reference.kind === "entity" ? reference.entityId : reference.referenceId) === id);
  ...
  const axisReference = (axis: "x" | "y"): Extract<SketchReference, { kind: "external-line" }> => axis === "x"
    ? { kind: "external-line", referenceId: "sketch-axis:x", a: { x: viewX, y: 0 }, b: { x: viewX + planeWidth, y: 0 }, source: "sketch-axis" }
    : { kind: "external-line", referenceId: "sketch-axis:y", a: { x: 0, y: viewY }, b: { x: 0, y: viewY + planeHeight }, source: "sketch-axis" };
  const axisOrLineAt = (axis: "x" | "y", position: Point) => preferredAxisOrSketchLineTarget(position, axis, entities, 14 / safeZoom, projectPoint);
  ...
  const choosePatternDirection = (reference: PatternDirectionReference) => {
    ...
    if (Math.hypot(reference.fallback.x, reference.fallback.y) < 1e-9) { setPatternMessage("Select an edge with a nonzero direction in the sketch plane, or a red/green axis"); return; }
    if (patternDraft.stage === "direction-1") {
      if (patternDraft.kind === "rectangular-pattern") { setPatternDraft({ ...patternDraft, direction1: reference, stage: "direction-2" }); setPatternMessage("Direction 1 selected · preview shown · select Direction 2 to expand the pattern"); }
  ...
  const patternPreviewEntities = useMemo(() => {
    if (!patternDraft || patternDraft.stage === "entities" || patternDraft.kind === "circular-pattern" && patternDraft.stage !== "parameters") return [];
    ...
    const columns = Math.max(2, Math.min(100, Math.round(patternDraft.count1))); const rows = patternDraft.kind === "rectangular-pattern" && patternDraft.direction2 ? Math.max(2, Math.min(100, Math.round(patternDraft.count2))) : 1;
  ...
  const handleEntityPointerDown = (event: React.PointerEvent<SVGGElement>, entity: SketchEntity) => {
    ...
    if (isPatternTool(tool) && patternDraft) {
      ...
      if ((patternDraft.stage === "direction-1" || patternDraft.stage === "direction-2") && entity.type === "line") { choosePatternDirection(directionReferenceForLine(entity)); return; }
      setPatternMessage(patternDraft.stage.startsWith("direction") ? "Select a straight sketch line, background edge, or red/green axis" : "Select the pattern reference"); return;
    }
    if (tool === "mirror") {
      event.preventDefault(); event.stopPropagation();
    ...
    if (tool === "tangent-constraint") {
      event.preventDefault(); event.stopPropagation();
      if (entity.type !== "line" && entity.type !== "circle") { setTangentMessage("Tangent needs one circle and one straight sketch line"); return; }
      if (!tangentSource) { setTangentSource(entity.id); setSelected(entity.id); setTangentMessage(entity.type === "circle" ? "Select the tangent line" : "Select the circle"); return; }
      if (tangentSource === entity.id) return;
      const first = entities.find(e => e.id === tangentSource);
      if (first) addTangent(first, entity);
      return;
    }
  ...
  const addTangent = (first: SketchEntity, second: SketchEntity) => {
    const relation = makeTangentConstraint(first, second, `constraint-${crypto.randomUUID()}`);
    if (!relation) { setTangentMessage("Tangent needs one circle and one nonzero straight line"); return; }
    if (constraints.some(c => isTangentConstraint(c) && c.first.entityId === relation.first.entityId && c.second.entityId === relation.second.entityId)) { setTangentMessage("This circle and line are already tangent"); setTangentSource(null); return; }
    const nextConstraints = [...constraints, relation];
    const next = synchronizeLinkedGeometry(entities, entities, nextConstraints);
    if (nextConstraints.filter(isTangentConstraint).some(c => !tangentSatisfied(c, next))) { setTangentMessage("Tangent conflicts with an existing tangent relation; remove it or change the radius first"); return; }
    setEntitiesState(next); onChange?.(next);
    commitConstraints([...constraintsForGeometry(next), relation]);
    setTangentSource(null); clearSelection(); setTangentMessage("Tangent relation added · select another circle and line");
  };
  const startTangent = () => {
    const selected = entities.filter(e => selectedEntityIds.includes(e.id));
    chooseTool("tangent-constraint", { clear: true });
    if (selected.length === 2) addTangent(selected[0], selected[1]);
    else if (selected.length === 1 && (selected[0].type === "circle" || selected[0].type === "line")) {
      setTangentSource(selected[0].id); setSelected(selected[0].id);
      setTangentMessage(selected[0].type === "circle" ? "Select the tangent line" : "Select the circle");
    }
  };
  const SketchCommandButton = ({ active = false, className = "", disabled = false, icon, label, title, onClick, pressed }: { active?: boolean; className?: string; disabled?: boolean; icon: React.ReactNode; label: string; title: string; onClick: () => void; pressed?: boolean }) => (
  ...
  return <div className={`sketcher-shell ${viewRotated ? "view-rotated" : ""}`} style={highlightStyle}>
    <div className="sketch-commandbar">
      <div className="sketch-command-group admin-group" aria-label="Sketch administration tools"><b>Admin</b><div className="sketch-command-tools">
        ...
        <SketchCommandButton disabled={!activeHistory.canUndo} icon="↶" label="Undo" title="Undo last sketch edit" onClick={undo} />
        <SketchCommandButton disabled={!activeHistory.canRedo} icon="↷" label="Redo" title="Redo sketch edit" onClick={redo} />
        ...
        <SketchCommandButton active={viewRotated} disabled={false} icon="⟂" label="Normal" title="Snap normal: nearest perpendicular sketch view, preserving zoom and roll" onClick={() => onSnapNormal?.()} />
      ...
      <div className="sketch-command-group geometry-group" aria-label="Sketch geometry tools"><b>Geometry</b><div className="sketch-command-tools">
        ...
        <RectangleToolMenu active={tool === "rectangle"} mode={rectangleMode} onChoose={(mode) => { setRectangleMode(mode); chooseTool("rectangle"); }} />
      ...
      <div className="sketch-command-group constraint-group" aria-label="Sketch constraint tools"><b>Constraints</b><div className="sketch-command-tools">
        ...
        <SketchCommandButton active={tool === "tangent-constraint"} className="constraint-tool" icon={<svg viewBox="0 0 24 24" width="24" height="24" fill="none" stroke="currentColor" strokeWidth="1.5"><circle cx="12" cy="9" r="6"/><path d="M2 15h20"/></svg>} label="Tangent" title="Tangent constraint: select a circle and a straight line, in either order" onClick={startTangent} />
    ...
    <svg ref={svgRef} onPointerDownCapture={(event) => { if (planeEdgeOn && event.button === 0) { event.preventDefault(); event.stopPropagation(); } }} className={`sketch-canvas ${planeEdgeOn ? "plane-edge-on" : ""} tool-${tool} ${dragging ? "dragging-point" : ""} ${draggingDimension ? "dragging-dimension" : ""} ${selectionMarquee ? "marquee-selecting" : ""} ${draggingSelection ? "dragging-selection" : ""} ${trimGesture ? "trimming-gesture" : ""}`} viewBox={`${rootX} ${rootY} ${viewWidth} ${viewHeight}`} preserveAspectRatio="xMidYMid slice" onPointerMove={onMove} onPointerUp={finishDrag} onPointerCancel={finishDrag} onPointerDown={onCanvasPointerDown} onDoubleClick={() => tool === "spline" && finishSpline()} onContextMenu={(event) => event.preventDefault()}>
      <g className="sketch-plane-content" transform={`matrix(${projection.join(" ")})`}>
      ...
      {showGrid && <rect x={viewX} y={viewY} width={planeWidth} height={planeHeight} fill="url(#major-grid)" />}
      <line x1={viewX} y1="0" x2={viewX + planeWidth} y2="0" className={`sketch-axis x ${patternReferenceChosen("external", "sketch-axis:x") ? "pattern-reference-selected" : ""} ${hoveredReferenceId === "sketch-axis:x" || xAxisChosen ? "reference-highlighted" : ""}`}/><line x1="0" y1={viewY} x2="0" y2={viewY + planeHeight} className={`sketch-axis y ${patternReferenceChosen("external", "sketch-axis:y") ? "pattern-reference-selected" : ""} ${hoveredReferenceId === "sketch-axis:y" || yAxisChosen ? "reference-highlighted" : ""}`}/>
      ...
      {entities.map((entity) => { const referenceId = `line:${entity.id}`; const dimensionLineTool = tool === "tangent-constraint" || tool === "linear-dimension" || tool === "angular-dimension" || tool === "perpendicular-constraint" || tool === "mirror" || isPatternTool(tool) && patternDraft?.stage.startsWith("direction"); const referenceChosen = dimensionReferences.some((reference) => reference.kind === "line" && reference.entityId === entity.id); const checkOpen = sketchCheck?.affectedEntityIds.includes(entity.id); return <g key={entity.id} className={`sketch-entity ${selectedEntityIds.includes(entity.id) ? "selected" : ""} ${checkOpen ? "sketch-check-open" : ""} ${extendSource?.entityId === entity.id || cornerSource?.entityId === entity.id || radiusSelections.some((selection) => selection.entityId === entity.id) || perpendicularSource === entity.id ? "extend-source" : ""} ${entity.construction ? "construction" : ""} ${dimensionLineTool && entity.type === "line" || tool === "diameter-dimension" && entity.type === "circle" || tool === "radial-dimension" && (entity.type === "circle" || entity.type === "arc") ? "constraint-selectable" : ""}`} onContextMenu={(event) => openConstraintShortcut(event, entity)} onPointerDown={(event) => handleEntityPointerDown(event, entity)}>
        ...
        {entitySnapPoints(entity).map((candidate, index) => <circle key={index} className={`sketch-point ${candidate.kind}`} transform={billboardAround(candidate.point)} cx={candidate.point.x} cy={candidate.point.y} r={nodeRadius} />)}
      </g>; })}
      ...
      {sketchCheck?.openEndpoints.map((endpoint, index) => <g key={`sketch-check-endpoint-${index}`} className="sketch-check-endpoint" transform={billboard(endpoint.point, 1 / safeZoom)} aria-label={`Open endpoint ${index + 1}`}><circle r="7"/><path d="M -3.5 -3.5 L 3.5 3.5 M 3.5 -3.5 L -3.5 3.5"/></g>)}
      {tool === "select" && entities.map((entity) => <g key={`controls-${entity.id}`} className={`control-layer ${selectedEntityIds.includes(entity.id) ? "active" : "inactive"}`}>{controlPointsForEntity(entity).map((control) => <circle key={control.handle} className="control-point" transform={billboardAround(control.point)} cx={control.point.x} cy={control.point.y} r={nodeRadius * (selectedEntityIds.includes(entity.id) ? 1.25 : 1)} onPointerDown={(event) => beginDrag(event, entity.id, control.handle, control.point)} onContextMenu={entity.type === "spline" ? (event) => openConstraintShortcut(event, entity) : undefined} />)}</g>)}
      ...
      {(tool === "linear-dimension" || tool === "angular-dimension" || isPatternTool(tool) && patternDraft?.stage.startsWith("direction")) && <g className="axis-reference-layer">
        <rect x={viewX} y={-11 / safeZoom} width={planeWidth} height={22 / safeZoom} className="axis-reference-hit x" role="button" tabIndex={0} aria-label="Red sketch X axis" onPointerEnter={(event) => hoverAxisOrLine("x", event)} onPointerMove={(event) => hoverAxisOrLine("x", event)} onPointerLeave={() => setHoveredReferenceId(null)} onPointerDown={(event) => isPatternTool(tool) ? choosePatternAxisOrLine("x", event) : chooseAxisOrLine("x", event)}/>
        <rect x={-11 / safeZoom} y={viewY} width={22 / safeZoom} height={planeHeight} className="axis-reference-hit y" role="button" tabIndex={0} aria-label="Green sketch Y axis" onPointerEnter={(event) => hoverAxisOrLine("y", event)} onPointerMove={(event) => hoverAxisOrLine("y", event)} onPointerLeave={() => setHoveredReferenceId(null)} onPointerDown={(event) => isPatternTool(tool) ? choosePatternAxisOrLine("y", event) : chooseAxisOrLine("y", event)}/>
      </g>}
      {tool === "linear-dimension" && entities.map((entity) => <g key={`constraint-nodes-${entity.id}`} className="constraint-node-layer">{controlPointsForEntity(entity).map((control) => { const reference: SketchReference = { kind: "node", entityId: entity.id, handle: control.handle }; const referenceId = `node:${entity.id}:${control.handle}`; const chosen = dimensionReferences.some((candidate) => candidate.kind === "node" && candidate.entityId === reference.entityId && candidate.handle === reference.handle); const otherPoints = dimensionControlPoints.filter((candidate) => candidate.entityId !== entity.id || candidate.handle !== control.handle).map((candidate) => candidate.point); const preferredHitRadius = Math.max(nodeRadius, (entity.type === "line" ? 10 : 16) / safeZoom); const hitRadius = nonOverlappingReferenceHitRadius(projectPoint(control.point), otherPoints.map(projectPoint), preferredHitRadius, entity.type === "line" ? 0.22 : 0.45); const chooseNode = (event: React.PointerEvent<SVGCircleElement>) => { if (event.button !== 0) return; event.stopPropagation(); chooseDimensionReference(reference, pointFromEvent(event)); }; return <g key={control.handle}><circle className="constraint-node-hit" transform={billboardAround(control.point)} role="button" tabIndex={0} aria-label={`Dimension node ${entity.id} ${control.handle}`} cx={control.point.x} cy={control.point.y} r={hitRadius} onPointerEnter={() => setHoveredReferenceId(referenceId)} onPointerLeave={() => setHoveredReferenceId((current) => current === referenceId ? null : current)} onPointerDown={chooseNode} onKeyDown={(event) => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); chooseDimensionReference(reference, control.point); } }} /><circle className={`constraint-node ${hoveredReferenceId === referenceId || chosen ? "reference-highlighted" : ""}`} transform={billboardAround(control.point)} cx={control.point.x} cy={control.point.y} r={nodeRadius} /></g>; })}</g>)}
      ...
      {constraints.map((constraint) => {
        if (!isDimensionConstraint(constraint)) return null;
        if (constraint.type === "diameter") {
        ...
        if (constraint.type === "radial") {
          ...
          return <g key={constraint.id} className={`linear-constraint radial-constraint ${constraint.conflicted ? "conflicted" : ""} ${selectedConstraintIds.includes(constraint.id) ? "selected" : ""}`}><line className="constraint-measure" x1={layout.first.x} y1={layout.first.y} x2={layout.second.x} y2={layout.second.y}/><path className="constraint-arrow" d={dimensionArrowPath(layout.first, layout.second, dimensionScale)}/><line className="constraint-extension" x1={layout.second.x} y1={layout.second.y} x2={layout.label.x} y2={layout.label.y}/><g className="constraint-label" role="button" tabIndex={0} aria-label={`Radial dimension ${text}${constraint.conflicted ? ", over defined" : ""}`} transform={billboard(layout.label, dimensionLabelScale)} onPointerDown={(event) => beginConstraintDrag(event, constraint)} onDoubleClick={(event) => { event.preventDefault(); event.stopPropagation(); setSelectedConstraintId(constraint.id); setEditingDimension(null); setEditingConstraintId(constraint.id); setDimensionValue(String(Number(editableValue.toFixed(4)))); }} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); setEditingConstraintId(constraint.id); setDimensionValue(String(Number(editableValue.toFixed(4)))); } }}><rect x={-labelWidth / 2} y="-7" width={labelWidth} height="14" rx="3"/><text textAnchor="middle" dominantBaseline="central">{text}</text></g></g>;
        }
        if (constraint.type === "angular") {
          ...
          return <g key={constraint.id} className={`linear-constraint angular-constraint ${constraint.conflicted ? "conflicted" : ""} ${selectedConstraintIds.includes(constraint.id) ? "selected" : ""}`}><line className="constraint-extension" x1={layout.vertex.x} y1={layout.vertex.y} x2={layout.firstRay.x} y2={layout.firstRay.y}/><line className="constraint-extension" x1={layout.vertex.x} y1={layout.vertex.y} x2={layout.secondRay.x} y2={layout.secondRay.y}/><path className="constraint-measure" d={`M ${layout.firstRay.x} ${layout.firstRay.y} A ${layout.radius} ${layout.radius} 0 ${layout.largeArc ? 1 : 0} ${layout.sweep ? 1 : 0} ${layout.secondRay.x} ${layout.secondRay.y}`}/><g className="constraint-label" role="button" tabIndex={0} aria-label={`Angular dimension ${text}${constraint.conflicted ? ", over defined" : ""}`} transform={billboard(layout.label, dimensionLabelScale)} onPointerDown={(event) => beginConstraintDrag(event, constraint)} onDoubleClick={(event) => { event.preventDefault(); event.stopPropagation(); setSelectedConstraintId(constraint.id); setEditingDimension(null); setEditingConstraintId(constraint.id); setDimensionValue(String(Number(displayedValue.toFixed(4)))); }} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); setEditingConstraintId(constraint.id); setDimensionValue(String(Number(displayedValue.toFixed(4)))); } }}><rect x={-labelWidth / 2} y="-7" width={labelWidth} height="14" rx="3"/><text textAnchor="middle" dominantBaseline="central">{text}</text></g></g>;
        }
        const layout = linearDimensionLayout(constraint.first, constraint.second, constraint.orientation, constraint.position, entities); const actualValue = linearDimensionValue(constraint.first, constraint.second, constraint.orientation, entities); const displayedValue = constraint.conflicted ? actualValue : constraint.value; const text = formatLength(displayedValue, unitSystem); const editableValue = fromMillimeters(displayedValue, unitSystem); const labelWidth = Math.max(38, text.length * 4.3 + 13); return <g key={constraint.id} className={`linear-constraint ${constraint.conflicted ? "conflicted" : ""} ${selectedConstraintIds.includes(constraint.id) ? "selected" : ""}`}><line className="constraint-extension" x1={layout.first.x} y1={layout.first.y} x2={layout.dimensionFirst.x} y2={layout.dimensionFirst.y}/><line className="constraint-extension" x1={layout.second.x} y1={layout.second.y} x2={layout.dimensionSecond.x} y2={layout.dimensionSecond.y}/><line className="constraint-measure" x1={layout.dimensionFirst.x} y1={layout.dimensionFirst.y} x2={layout.dimensionSecond.x} y2={layout.dimensionSecond.y}/><path className="constraint-arrow" d={dimensionArrowPath(layout.dimensionFirst, layout.dimensionSecond, dimensionScale)}/><g className="constraint-label" role="button" tabIndex={0} aria-label={`Linear dimension ${text}${constraint.conflicted ? ", over defined" : ""}`} transform={billboard(layout.label, dimensionLabelScale)} onPointerDown={(event) => beginConstraintDrag(event, constraint)} onDoubleClick={(event) => { event.preventDefault(); event.stopPropagation(); setSelectedConstraintId(constraint.id); setEditingDimension(null); setEditingConstraintId(constraint.id); setDimensionValue(String(Number(editableValue.toFixed(4)))); }} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); setEditingConstraintId(constraint.id); setDimensionValue(String(Number(editableValue.toFixed(4)))); } }}><rect x={-labelWidth / 2} y="-7" width={labelWidth} height="14" rx="3"/><text textAnchor="middle" dominantBaseline="central">{text}</text></g></g>;
      })}
      {linearPreview && <g className="linear-constraint preview"><line className="constraint-extension" x1={linearPreview.first.x} y1={linearPreview.first.y} x2={linearPreview.dimensionFirst.x} y2={linearPreview.dimensionFirst.y}/><line className="constraint-extension" x1={linearPreview.second.x} y1={linearPreview.second.y} x2={linearPreview.dimensionSecond.x} y2={linearPreview.dimensionSecond.y}/><line className="constraint-measure" x1={linearPreview.dimensionFirst.x} y1={linearPreview.dimensionFirst.y} x2={linearPreview.dimensionSecond.x} y2={linearPreview.dimensionSecond.y}/><path className="constraint-arrow" d={dimensionArrowPath(linearPreview.dimensionFirst, linearPreview.dimensionSecond, dimensionScale)}/><g className="constraint-preview-label" transform={billboard(linearPreview.label, dimensionLabelScale)}><rect x="-32" y="-12" width="64" height="24" rx="4"/><text y="-2" textAnchor="middle">{formatLength(linearPreview.value, unitSystem)}</text><text y="7" textAnchor="middle" className="orientation-hint">{dimensionOrientation} · Tab</text></g></g>}
      {angularPreview && <g className="linear-constraint angular-constraint preview"><line className="constraint-extension" x1={angularPreview.vertex.x} y1={angularPreview.vertex.y} x2={angularPreview.firstRay.x} y2={angularPreview.firstRay.y}/><line className="constraint-extension" x1={angularPreview.vertex.x} y1={angularPreview.vertex.y} x2={angularPreview.secondRay.x} y2={angularPreview.secondRay.y}/><path className="constraint-measure" d={`M ${angularPreview.firstRay.x} ${angularPreview.firstRay.y} A ${angularPreview.radius} ${angularPreview.radius} 0 ${angularPreview.largeArc ? 1 : 0} ${angularPreview.sweep ? 1 : 0} ${angularPreview.secondRay.x} ${angularPreview.secondRay.y}`}/><g className="constraint-preview-label" transform={billboard(angularPreview.label, dimensionLabelScale)}><rect x="-32" y="-10" width="64" height="20" rx="4"/><text textAnchor="middle" dominantBaseline="central">{`${angularPreview.value.toFixed(1).replace(/\.0$/, "")}°`}</text></g></g>}
      {diameterPreview && <g className="linear-constraint diameter-constraint preview"><line className="constraint-measure" x1={diameterPreview.first.x} y1={diameterPreview.first.y} x2={diameterPreview.second.x} y2={diameterPreview.second.y}/><path className="constraint-arrow" d={dimensionArrowPath(diameterPreview.first, diameterPreview.second, dimensionScale)}/><line className="constraint-extension" x1={diameterPreview.second.x} y1={diameterPreview.second.y} x2={diameterPreview.label.x} y2={diameterPreview.label.y}/><g className="constraint-preview-label" transform={billboard(diameterPreview.label, dimensionLabelScale)}><rect x="-32" y="-10" width="64" height="20" rx="4"/><text textAnchor="middle" dominantBaseline="central">{`⌀ ${formatLength(diameterPreview.value, unitSystem)}`}</text></g></g>}
      {radialPreview && <g className="linear-constraint radial-constraint preview"><line className="constraint-measure" x1={radialPreview.first.x} y1={radialPreview.first.y} x2={radialPreview.second.x} y2={radialPreview.second.y}/><path className="constraint-arrow" d={dimensionArrowPath(radialPreview.first, radialPreview.second, dimensionScale)}/><line className="constraint-extension" x1={radialPreview.second.x} y1={radialPreview.second.y} x2={radialPreview.label.x} y2={radialPreview.label.y}/><g className="constraint-preview-label" transform={billboard(radialPreview.label, dimensionLabelScale)}><rect x="-32" y="-10" width="64" height="20" rx="4"/><text textAnchor="middle" dominantBaseline="central">{`R ${formatLength(radialPreview.value, unitSystem)}`}</text></g></g>}
      {visibleDimensions.map((dimension) => { const position = dimensionPosition(dimension); const text = dimensionTextFor(dimension); const editableValue = fromMillimeters(dimension.value, unitSystem); const labelWidth = Math.max(36, text.length * 4.3 + 13); const moved = dimensionOffsets[dimension.key] && (Math.abs(dimensionOffsets[dimension.key].x) > 0.1 || Math.abs(dimensionOffsets[dimension.key].y) > 0.1); return <g key={dimension.key} className={`dimension-annotation ${selectedDimensionKeys.includes(dimension.key) ? "selected" : ""}`}>{moved && <line className="dimension-leader" x1={dimension.anchor.x} y1={dimension.anchor.y} x2={position.x} y2={position.y}/>}<g className="sketch-dimension" role="button" tabIndex={0} aria-label={`Edit dimension ${text}`} transform={billboard(position, dimensionLabelScale)} onPointerDown={(event) => beginDimensionDrag(event, dimension)} onDoubleClick={(event) => { event.preventDefault(); event.stopPropagation(); setTool("select"); setEditingConstraintId(null); setSelectedConstraintId(null); setEditingDimension(dimension); setDimensionValue(String(Number(editableValue.toFixed(4)))); }} onKeyDown={(event) => { if (event.key === "Enter") { event.preventDefault(); setEditingConstraintId(null); setSelectedConstraintId(null); setEditingDimension(dimension); setDimensionValue(String(Number(editableValue.toFixed(4)))); } }}><line x1={-labelWidth / 2 - 5} y1="0" x2={labelWidth / 2 + 5} y2="0"/><path d={`M${-labelWidth / 2 - 5} 0 l4 -2 v4z M${labelWidth / 2 + 5} 0 l-4 -2 v4z`}/><rect x={-labelWidth / 2} y="-7" width={labelWidth} height="14" rx="3"/><text textAnchor="middle" dominantBaseline="central">{text}</text></g></g>; })}
      {selectionMarquee && marqueeBox && <rect transform={billboard(unprojectSketchPoint({ x: 0, y: 0 }, projection) ?? { x: 0, y: 0 }, 1)} className={`sketch-selection-box ${projectPoint(selectionMarquee.current).x < projectPoint(selectionMarquee.start).x ? "crossing" : "window"}`} x={marqueeBox.left} y={marqueeBox.top} width={marqueeBox.right - marqueeBox.left} height={marqueeBox.bottom - marqueeBox.top}/>}
      {entities.filter((entity): entity is Extract<SketchEntity, { type: "line" }> => entity.type === "line" && Boolean(entity.axisConstraint)).map((entity) => { const relation = entity.axisConstraint!; const center = midpoint(entity.a, entity.b); const dx = entity.b.x - entity.a.x; const dy = entity.b.y - entity.a.y; const length = Math.max(distance(entity.a, entity.b), 0.001); const position = { x: center.x - dy / length * 10 / safeZoom, y: center.y + dx / length * 10 / safeZoom }; const selectedRelation = selectedAxisConstraint?.entityId === entity.id && selectedAxisConstraint.relation === relation; return <g key={`locked-${entity.id}`} role="button" tabIndex={0} aria-label={`${relation} constraint; select and press Delete to remove`} transform={billboard(position, 1 / safeZoom)} className={`locked-axis-constraint ${selectedRelation ? "selected" : ""}`} onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); clearSelection(); setSelectedAxisConstraint({ entityId: entity.id, relation }); setTool("select"); }}><rect x="-5" y="-5" width="10" height="10" rx="2"/><text textAnchor="middle" dominantBaseline="central">{relation === "Horizontal" ? "H" : "V"}</text></g>; })}
      {constraints.filter(isTangentConstraint).map(constraint => {
        const contact = tangentContact(constraint, entities); if (!contact) return null;
        const selected = selectedConstraintIds.includes(constraint.id);
        const choose = () => { clearSelection(); setSelectedConstraintId(constraint.id); setTool("select"); };
        return <g key={constraint.id} role="button" tabIndex={0} aria-label="Tangent constraint; select and press Delete to remove" transform={billboard({ x: contact.x + 8 / safeZoom, y: contact.y + 8 / safeZoom }, 1 / safeZoom)} className={`tangent-constraint-badge mirror-constraint-badge ${selected ? "selected" : ""} ${tangentSatisfied(constraint, entities) ? "" : "conflicted"}`} onPointerDown={event => { if (event.button !== 0) return; event.preventDefault(); event.stopPropagation(); choose(); }} onKeyDown={event => { if (event.key === "Enter" || event.key === " ") { event.preventDefault(); choose(); } }}>
          <title>Tangent — select and Delete to release</title><rect x="-5" y="-5" width="10" height="10" rx="1.5"/><circle cx="0" cy="-1" r="2.5" fill="none"/><path d="M-4 1.5h8" fill="none"/>
        </g>;
      })}
      {constraints.filter((constraint): constraint is MirrorConstraint => constraint.type === "mirror").flatMap((constraint) => constraint.pairs.map((pair, index) => {
        ...
        return <g key={`${constraint.id}-${index}`} role="button" tabIndex={0} aria-label="Mirror constraint; select and press Delete to unlink" transform={billboard({ x: anchor.x + 8 / safeZoom, y: anchor.y - 8 / safeZoom }, 1 / safeZoom)} className={`mirror-constraint-badge ${selectedMirror ? "selected" : ""}`} onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); clearSelection(); setSelectedConstraintId(constraint.id); setTool("select"); }}><rect x="-4" y="-4" width="8" height="8" rx="1.5"/><text textAnchor="middle" dominantBaseline="central">M</text></g>;
      }))}
      {constraints.filter(isPatternConstraint).flatMap((constraint) => constraint.pairs.flatMap((pair) => pair.instances.map((instance, index) => {
        ...
        return <g key={`${constraint.id}-${pair.sourceId}-${index}`} role="button" tabIndex={0} aria-label={`${constraint.type.replace("-pattern", "")} pattern constraint; select and press Delete to unlink`} transform={billboard({ x: anchor.x + 7 / safeZoom, y: anchor.y - 7 / safeZoom }, 1 / safeZoom)} className={`pattern-constraint-badge ${selectedPattern ? "selected" : ""}`} onPointerDown={(event) => { event.preventDefault(); event.stopPropagation(); clearSelection(); setSelectedConstraintId(constraint.id); setTool("select"); }}><circle r="3.2"/><text textAnchor="middle" dominantBaseline="central">{glyph}</text></g>;
      })))}
